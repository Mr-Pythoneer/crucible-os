...
- [x] **Website GPU configurator (2026-07-01)** — the deferred "pick your GPU → get a tailored build" flow is built: `docs/index.html` `#configure` section is an interactive static widget mapping GPU → effective VRAM → tier (same thresholds as `distro-ai-detect-tier`), showing the per-use-case preloaded models and routing datacenter GPUs to Server mode. Site also refreshed off the stale Crucible12/llama.cpp copy onto LM Studio + ComfyUI. See `docs/website-configurator-design.md` (now marked BUILT).
