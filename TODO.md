...
- [x] `distro-ai-model` use-case switcher (list/use/load/server/status/unload) — replaces `distro-ai-preset`; tier/profile/VRAM-aware; execution-tested with a **stubbed `lms`** (`tests/test_ai_model.sh`, 24 assertions; never touches a real LM Studio)
