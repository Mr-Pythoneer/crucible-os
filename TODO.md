...
- [ ] **QEMU boot smoke test in CI** — the ISO exists but has NEVER BOOTED anywhere. GitHub Linux runners have KVM; a follow-up job can boot the ISO headless (BIOS/SeaBIOS — the fork's output is BIOS-only, no EFI support exists in 3.0~a57) and assert the live system comes up. This closes "built but unbooted" with zero local hardware.
